...
# Features: 系统代理集成、透明代理、增强工具支持、代理Rule management
...
"@
    ...
    Write-ColorOutput "Default proxy rules created: $script:RulesFile" "Success"
}
        ...
        switch ($Action.ToLower()) {
            "enable" {
                if (![string]::IsNullOrEmpty($HttpProxy)) {
                    ...
                    Write-ColorOutput "Windows system HTTP proxy enabled: $HttpProxy" "Success"
                }
                
                if (![string]::IsNullOrEmpty($SocksProxy)) {
                    # SOCKS proxy setting is more complex, requires ProxyServer string format
                    ...
                    Set-ItemProperty -Path $registryPath -Name "ProxyEnable" -Value 1
                    Write-ColorOutput "Windows system SOCKS proxy enabled: $SocksProxy" "Success"
                }
                
                # Set proxy override list
            ...
            "disable" {
                ...
                Write-ColorOutput "Windows system proxy disabled" "Success"
            }
        }
...
'@
        ...
        Write-ColorOutput "Failed to configure Windows system proxy: $($_.Exception.Message)" "Error"
    }
}
...
function Set-EnhancedToolsProxy {
    ...
    switch ($Action.ToLower()) {
        "enable" {
            Write-ColorOutput "Configuring enhanced tool proxy..." "Info"
            ...
            if (Get-Command git -ErrorAction SilentlyContinue) {
                if (![string]::IsNullOrEmpty($HttpProxy)) {
                    ...
                    Write-ColorOutput "Git proxy configured: $HttpProxy" "Success"
                }
            }
            ...
            if (Get-Command npm -ErrorAction SilentlyContinue) {
                if (![string]::IsNullOrEmpty($HttpProxy)) {
                    ...
                    Write-ColorOutput "NPM proxy configured: $HttpProxy" "Success"
                }
            }
            ...
            if (Get-Command docker -ErrorAction SilentlyContinue) {
                ...
                Write-ColorOutput "Docker proxy configured" "Success"
            }
        ...
        "disable" {
            Write-ColorOutput "Disabling enhanced tool proxy..." "Info"
            ...
            Write-ColorOutput "Enhanced tool proxy disabled" "Success"
        }
    }
}
...
'@
    ...
    Write-ColorOutput "Command wrappers created: $wrapperFile" "Success"
}

# Transparent proxy support (Windows)
    ...
    switch ($Action.ToLower()) {
        "enable" {
            if (!(Test-Administrator)) {
                Write-ColorOutput "Transparent proxy requires administrator privileges, please run as administrator" "Warning"
                return
            }
            
            Write-ColorOutput "Windows transparent proxy requires third-party tool support" "Info"
            Write-ColorOutput "Recommended: Install Proxifier, SocksCap64, or v2rayN" "Info"
            
            # Third-party tool integration can be added here
        }
        
        "disable" {
            Write-ColorOutput "Transparent proxy disabled" "Success"
        }
    }
}

# Main proxy control function
    ...
    switch ($Action.ToLower()) {
        "enable" {
            ...
            if (!(Test-IPAddress $IP) -or !(Test-Port $Port)) {
                Write-ColorOutput "Invalid IP address or port: $IP:$Port" "Error"
                return
            }
            
            Write-ColorOutput "Enabling proxy: $Protocol mode, $IP:$Port" "Info"
            ...
            Write-ColorOutput "Proxy enabled" "Success"
        }
        
        "disable" {
            Write-ColorOutput "Disabling proxy: $Protocol" "Info"
            ...
            Write-ColorOutput "Proxy disabled" "Success"
        }
    }
}
    ...
    switch ($RuleAction.ToLower()) {
        "list" {
            if (Test-Path $script:RulesFile) {
                Write-Host "`n=== Proxy rules ===" -ForegroundColor Cyan
                Get-Content $script:RulesFile | Where-Object { $_ -notmatch '^#' -and $_ -ne '' } | ForEach-Object {
                    Write-Host $_ -ForegroundColor White
                }
            } else {
                Write-ColorOutput "Rules file does not exist, please run proxy rules init" "Warning"
            }
        }
        ...
        "edit" {
            if (Test-Path $script:RulesFile) {
                if (Get-Command notepad -ErrorAction SilentlyContinue) {
                    ...
                    Write-ColorOutput "Please manually edit file: $script:RulesFile" "Info"
                }
            } else {
                Write-ColorOutput "Rules file does not exist, please first run proxy rules init" "Warning"
            }
        }
    }
}
...
function Install-ProxyDependencies {
    Write-ColorOutput "Checking PowerShell proxy dependencies..." "Info"
    
    # 检查PowerShell版本
    if ($PSVersionTable.PSVersion.Major -lt 5) {
        Write-ColorOutput "Recommend upgrading to PowerShell 5.0or higher version" "Warning"
    } else {
        Write-ColorOutput "PowerShellVersion check passed: $($PSVersionTable.PSVersion)" "Success"
    }
    
    # 检查curl
    if (Get-Command curl -ErrorAction SilentlyContinue) {
        Write-ColorOutput "curl installed" "Success"
    } else {
        Write-ColorOutput "curl not found, some features may be limited" "Warning"
    }
    
    # 检查git
    if (Get-Command git -ErrorAction SilentlyContinue) {
        Write-ColorOutput "Git installed" "Success"
    } else {
        Write-ColorOutput "Git not found, Git proxy feature will be unavailable" "Warning"
    }
    
    Write-ColorOutput "Dependency check completed" "Success"
    Write-ColorOutput "Recommended: Install Proxifier or SocksCap64 for better transparent proxy support" "Info"
}
...
function Generate-CurlTestCommands {
    ...
    Write-Host "`n=== CURL Test Command Generator ===" -ForegroundColor Cyan
    
    if (!$state.PROXY_ENABLED) {
        Write-ColorOutput "Proxy not enabled, showing basic test commands" "Warning"
        Write-Host "`nBasic HTTP test:" -ForegroundColor Green
        Write-Host "curl http://httpbin.org/ip" -ForegroundColor White
        Write-Host "curl https://ip.sb" -ForegroundColor White
        return
    }
    
    Write-Host "`nCurrent proxy configuration:" -ForegroundColor Blue
    ...
    Write-Host "`n=== HTTP Proxy Test Commands ===" -ForegroundColor Yellow
    if ($state.CURRENT_HTTP_PROXY) {
        ...
        Write-Host "`n# Detailed information check" -ForegroundColor Green
        ...
        Write-Host "`n# Connection test" -ForegroundColor Green
    ...
    Write-Host "`n=== SOCKS5 Proxy Test Commands ===" -ForegroundColor Yellow
    if ($state.CURRENT_SOCKS_PROXY) {
        ...
        Write-Host "`n# DNS test (SOCKS5 specific)" -ForegroundColor Green
        Write-Host "curl --socks5-hostname $socksProxy https://dns.google/resolve?name=google.com&type=A" -ForegroundColor White
    }
    
    Write-Host "`n=== Automatic test execution ===" -ForegroundColor Yellow
    Write-Host "Do you want to execute automatic test？[Y/N]: " -NoNewline -ForegroundColor Cyan
    $response = Read-Host
    
    if ($response -eq 'Y' -or $response -eq 'y') {
        Write-Host "`nExecuting automatic test..." -ForegroundColor Cyan
        
        if ($state.CURRENT_SOCKS_PROXY) {
            Write-Host "`nTesting SOCKS5 proxy:" -ForegroundColor Green
            try {
                ...
                if ($result) {
                    ...
                    if ($ipInfo.origin) {
                        Write-Host "✓ SOCKS5 proxy working normally, current IP: $($ipInfo.origin)" -ForegroundColor Green
                    } else {
                        Write-Host "✓ SOCKS5 proxy connection successful, response: $result" -ForegroundColor Green
                    }
                } else {
                    Write-Host "✗ SOCKS5 proxy test failed" -ForegroundColor Red
                }
            } catch {
                Write-Host "✗ SOCKS5 proxy test error: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        
        if ($state.CURRENT_HTTP_PROXY) {
            Write-Host "`nTesting HTTP proxy:" -ForegroundColor Green
            try {
                $result = curl --proxy http://$($state.CURRENT_HTTP_PROXY) --max-time 10 -s http://httpbin.org/ip 2>$null
                if ($result) {
                    $ipInfo = $result | ConvertFrom-Json -ErrorAction SilentlyContinue
                    if ($ipInfo.origin) {
                        Write-Host "✓ HTTP proxy working normally, current IP: $($ipInfo.origin)" -ForegroundColor Green
                    } else {
                        Write-Host "✓ HTTP proxy connection successful, response: $result" -ForegroundColor Green
                    }
                } else {
                    Write-Host "✗ HTTP proxy test failed" -ForegroundColor Red
                }
            } catch {
                Write-Host "✗ HTTP proxy test error: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
    }
...
function Show-Help {
    ...
    Write-Host "`nRule management:" -ForegroundColor Blue
    ...
    Write-Host "   proxy curltest        # Generate curl test commands" -ForegroundColor Gray
    
    Write-Host "`n7. Rule management："
...
function Main {
    ...
    if ($Version) {
        ...
        Write-Host "新增功能: 系统代理集成、透明代理支持、增强工具集成、代理Rule management" -ForegroundColor Blue
        ...
        Write-Host "• 代理Rule management系统" -ForegroundColor Gray
